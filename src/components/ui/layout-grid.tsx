...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    <div className="mx-auto grid h-full w-full max-w-7xl grid-cols-1 gap-4 py-10 md:grid-cols-3">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, "")}>
          <motion.div
            ...
            className={cn(
              card.className,
              "relative overflow-hidden border-2 border-black hover:cursor-pointer",
              selected?.id === card.id
      ...
      <motion.div
        ...
        className={cn(
          "fixed left-0 top-0 z-10 h-screen w-screen overflow-hidden bg-black opacity-0",
